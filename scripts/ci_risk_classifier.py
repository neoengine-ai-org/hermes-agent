...
MATURITY_ORDER = [f"v{i}" for i in range(1, 11)]
RALPH_LOOP_STAGES = {
    "runtime_intent",
    "acceptance_boundary",
    "implementation_pass",
    "adversarial_pass",
    "runtime_proof_pass",
    "memory_closeout_pass",
}
SUPPORT_ONLY_CLASSIFICATIONS = {
    "evidence-only",
    "rubric-only",
    "maturity-only",
    "governance-only",
...
SURFACE_TO_CI = {
    ...
    "prd_packet": {"validate_prd_packets", "runtimePayloadContract_validation", "blocker_exemption_validation", "runtime_maturity_validation", "ralph_convergence_validation"},
    ...
    "product_intelligence": {"rubric_tests", "evaluation_score_drift_tests", "runtime_maturity_validation"},
}
...
class Classification:
    ...
    maturity_level_target: str | None = None
    current_maturity_level: str | None = None
    target_maturity_level: str | None = None
    ralph_loop_stage: str | None = None
    runtime_bearing: bool = False
    support_work_classification: str | None = None
...
def classify(files: list[str], body: str, additions: int = 0, pr_number: str = "unknown", repo: str = "unknown", title: str = "unknown") -> Classification:
    ...
    runtime_payload_declared = parse_boolish(body, "runtime_payload_required")
    runtime_payload_required = runtime_payload_declared == "yes"
    ...
    maturity_level_target = parse_declared_field(body, "maturityLevelTarget")
    current_maturity_level = parse_declared_field(body, "currentMaturityLevel")
    target_maturity_level = parse_declared_field(body, "targetMaturityLevel")
    ralph_loop_stage = parse_declared_field(body, "ralphLoopStage")
    support_work_classification = parse_declared_field(body, "supportWorkClassification")
    runtime_behavior_delta = parse_declared_field(body, "runtimeBehaviorDelta")
    runtime_bearing = bool((surfaces & RUNTIME_SURFACES) or runtime_payload_required or runtime_behavior_delta)
    ...
    if runtime_payload_declared not in {"yes", "no"}:
        blocking.append("missing_or_invalid_runtime_payload_required_declaration")
    ...
    if runtime_payload_required:
        required_payload_fields = {
            "Product/runtime artifact": "runtime_payload_required_without_product_runtime_artifact",
            "Changed runtime/product files": "runtime_payload_required_without_changed_runtime_product_files",
            "Behavior tests": "runtime_payload_required_without_behavior_tests",
            "Validation commands": "runtime_payload_required_without_validation_commands",
            "Explicit non-claims": "runtime_payload_required_without_explicit_non_claims",
        }
        for label, condition in required_payload_fields.items():
            if parse_declared_field(body, label) is None:
    ...
    if "prd_packet" in surfaces:
        required_prd_fields = {
            "maturityLevelTarget": "prd_packet_without_maturityLevelTarget",
            "currentMaturityLevel": "prd_packet_without_currentMaturityLevel",
            "targetMaturityLevel": "prd_packet_without_targetMaturityLevel",
            "ralphLoopStage": "prd_packet_without_ralphLoopStage",
            "productSurfaceTarget": "prd_packet_without_productSurfaceTarget",
            "runtimeBehaviorDelta": "prd_packet_without_runtimeBehaviorDelta",
            "userOrSystemVisibleDelta": "prd_packet_without_userOrSystemVisibleDelta",
            "validationTestDelta": "prd_packet_without_validationTestDelta",
            "runtimeProofRequired": "prd_packet_without_runtimeProofRequired",
            "finiteCloseCondition": "prd_packet_without_finiteCloseCondition",
            "nonClaims": "prd_packet_without_nonClaims",
            "nextRuntimePacketRecommendation": "prd_packet_without_nextRuntimePacketRecommendation",
        }
        for label, condition in required_prd_fields.items():
            if parse_declared_field(body, label) is None and label not in body:
                blocking.append(condition)
        if not runtime_contract_present:
            blocking.append("prd_packet_without_runtimePayloadContract")
        if maturity_level_target not in MATURITY_ORDER:
            blocking.append("missing_or_invalid_maturityLevelTarget")
        if current_maturity_level not in MATURITY_ORDER:
            blocking.append("missing_or_invalid_currentMaturityLevel")
        if target_maturity_level not in MATURITY_ORDER:
            blocking.append("missing_or_invalid_targetMaturityLevel")
        if ralph_loop_stage not in RALPH_LOOP_STAGES:
            blocking.append("missing_or_invalid_ralphLoopStage")

    support_lower = (support_work_classification or "").strip().lower()
    if support_lower in SUPPORT_ONLY_CLASSIFICATIONS:
        bound = parse_declared_field(body, "Bound runtime-bearing packet")
        finite_close = parse_declared_field(body, "finiteCloseCondition") or parse_declared_field(body, "Stop condition")
        if bound is None or finite_close is None:
            blocking.append("support_work_not_bound_to_runtime_packet")

    if target_maturity_level in MATURITY_ORDER:
        target_idx = MATURITY_ORDER.index(target_maturity_level) + 1
        delta_text = " ".join(filter(None, [runtime_behavior_delta, parse_declared_field(body, "userOrSystemVisibleDelta") or ""]))
        delta_lower = delta_text.lower()
        if target_idx >= 6 and not any(term in delta_lower for term in ("decision", "classif", "recommend", "prioriti", "rank", "rout", "diagnos", "intelligen")):
            blocking.append("v6_target_without_runtime_intelligence_delta")
        if target_idx >= 7 and not any(term in delta_lower for term in ("context", "history", "memory", "adapt", "state", "prior outcome")):
            blocking.append("v7_target_without_runtime_adaptation_delta")
        if target_idx >= 8 and not any(term in delta_lower for term in ("agent", "action", "autonom", "bounded", "authority", "escalat")):
            blocking.append("v8_target_without_bounded_autonomy_delta")
        if target_idx >= 9 and not any(term in delta_lower for term in ("cross", "handoff", "multiple", "ecosystem", "downstream", "org", "repository")):
            blocking.append("v9_target_without_runtime_ecosystem_delta")
        if target_idx >= 10 and not any(term in delta_lower for term in ("learn", "audit", "govern", "rollback", "regression", "self-improv")):
            blocking.append("v10_target_without_runtime_institution_delta")
    ...
    return Classification(
        ...
        maturity_level_target=maturity_level_target,
        current_maturity_level=current_maturity_level,
        target_maturity_level=target_maturity_level,
        ralph_loop_stage=ralph_loop_stage,
        runtime_bearing=runtime_bearing,
        support_work_classification=support_work_classification,
