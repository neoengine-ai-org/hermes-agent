...
@contextlib.contextmanager
def _bounded_cross_process_init_lock(path: Path):
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # Bound the corrupt-backup swarm before computing/copying a new
    # content-addressed artifact. Repeated crash-loop connects must not fill
    # the disk, and literal prefix matching keeps unusual filenames containing
    # glob metacharacters safe.
    cap = _corrupt_backup_cap()
    if cap > 0:
        backup_prefix = f"{base_name}.corrupt."
        try:
            existing = sum(
                1 for candidate in parent.iterdir()
...
def write_txn(conn: sqlite3.Connection):
    ...
    shadow the original exception with a spurious rollback error. The fork's
    cross-process sidecar lock and optional forensic events wrap the current
    upstream retry/invariant logic so offline swaps cannot race a writer.
    """
    with _cross_process_write_lock(conn):
        ...
        try:
            ...
            try:
                ...
                # SQLite may already have auto-rolled back after EIO,
                # contention, or corruption. Preserve the original error.
                pass
            ...
            # file pages. A discrepancy means a torn extend.
...
LANE_TERMINAL_CLAIM_STATUSES = {
    "completed", "blocked", "refused", "superseded",
    "no-op with evidence", "expired/recovered",
}
...
def _lane_row(conn: sqlite3.Connection, lane_id: str) -> Optional[dict[str, Any]]:
    return _dict(
        conn.execute(
            "SELECT * FROM lane_heartbeats WHERE lane_id=?", (lane_id,)
        ).fetchone()
    )
    ...
    *,
    ...
    if state == "idle-no-work":
        idle_backoff = min(
            max(
                prev_backoff * 2
                if previous and previous.get("state") == "idle-no-work"
                else 60,
                60,
            ),
            3600,
...
def discover_lane_session_state(
    conn: sqlite3.Connection, lane_id: str, agent_session_id: str
) -> dict[str, Any]:
    heartbeat = _lane_row(conn, lane_id)
    packet_row = conn.execute(
        "SELECT packet_json FROM lane_continuity_packets WHERE lane_id=?", (lane_id,)
    ).fetchone()
    ...
    if heartbeat and heartbeat.get("claimed_work_item_id"):
        active_claim = _dict(
            conn.execute(
                ...
                (heartbeat["claimed_work_item_id"],),
            ).fetchone()
        )
    return {
        "heartbeat": heartbeat,
        ...
        "ownership_valid": bool(
            active_claim and active_claim.get("claim_owner") == agent_session_id
        ),
    }
    ...
    *,
    ...
    with write_txn(conn):
        ...
        work_item = conn.execute(
            "SELECT * FROM lane_work_items WHERE work_item_id=?", (work_item_id,)
        ).fetchone()
        ...
        if work_item["status"] == "blocked" and not (
            ...
            and (
                work_item["blocked_event_id"] is None
                or work_item["last_event_id"] != work_item["blocked_event_id"]
            )
        ):
        ...
        try:
            cursor = conn.execute(
                ...
                """,
                (
                    work_item_id, lane_id, claim_owner, ts, expires,
                    evidence_path, recovery_of_claim_id,
                ),
            )
            update = conn.execute(
                "UPDATE lane_work_items SET status='claimed', updated_at=? "
                "WHERE work_item_id=? AND status IN ('open', 'blocked')",
                (ts, work_item_id),
            )
            ...
            return {
                ...
                "active_claim": dict(existing) if existing else None,
            }
    return {"claimed": True, "claim_id": cursor.lastrowid, "claim_expires_at": expires}
...
def close_lane_claim(
    ...
    *,
    ...
    ts = _now(now)
    status_by_close = {
        "completed": "completed", "blocked": "blocked", "refused": "refused",
        "superseded": "superseded", "no-op with evidence": "completed",
        "expired/recovered": "open",
    }
    with write_txn(conn):
        row = conn.execute(
            "SELECT work_item_id FROM lane_claims WHERE id=? AND claim_status='active'",
            (claim_id,),
        ).fetchone()
        result = conn.execute(
        ...
        if result.rowcount == 1 and row is not None:
            conn.execute(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        for row in rows:
            ...
            conn.execute(
                """
                UPDATE lane_work_items SET status='open', updated_at=?
                WHERE work_item_id=? AND status='claimed'
                  AND NOT EXISTS (
...
def record_lane_event(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        if work_item_id:
            conn.execute(
                "UPDATE lane_work_items SET last_event_id=?, updated_at=? WHERE work_item_id=?",
                (event_id, ts, work_item_id),
            )
    return {"event_id": event_id, "event_type": event_type, "work_item_id": work_item_id}


def next_lane_wake(
    conn: sqlite3.Connection, lane_id: str, *, now: Optional[int] = None
) -> dict[str, Any]:
    ts = _now(now)
    heartbeat = _lane_row(conn, lane_id)
    last_event_id = int(heartbeat.get("last_event_id") or 0) if heartbeat else 0
    event = conn.execute(
        "SELECT * FROM lane_events WHERE (lane_id=? OR lane_id IS NULL) "
        "AND id > ? AND consumed_at IS NULL ORDER BY id LIMIT 1",
    ...
    wake_time = int(heartbeat.get("next_eligible_wake_time") or ts) if heartbeat else ts
    return {
        "eligible_now": ts >= wake_time,
        "wake_reason": "timer",
        "next_eligible_wake_time": wake_time,
    }
    ...
    with write_txn(conn):
        conn.execute(
            ...
            ON CONFLICT(work_item_id) DO UPDATE SET
                repo_scope=excluded.repo_scope, status=excluded.status,
                priority=excluded.priority,
                failed_required_ci=excluded.failed_required_ci,
                stale_open_pr=excluded.stale_open_pr,
                governance_blocker=excluded.governance_blocker,
                review_gap=excluded.review_gap,
                dependency_unblocked=excluded.dependency_unblocked,
            ...
            """,
            (
                work_item_id, repo_scope, status, int(priority),
                int(failed_required_ci), int(stale_open_pr),
                int(governance_blocker), int(review_gap),
                int(dependency_unblocked), governance_state, capability,
                blocked_event_id, ts, ts,
            ),
        )
    return _dict(
        conn.execute(
...
def _work_item_pickable(
    ...
    if row["status"] == "blocked" and not (
        row["last_event_id"]
        and (
            row["blocked_event_id"] is None
            or row["last_event_id"] != row["blocked_event_id"]
        )
    ):
        return False
    if conn.execute(
        "SELECT 1 FROM lane_claims WHERE work_item_id=? AND claim_status='active'",
        (row["work_item_id"],),
    ).fetchone():
        return False
    return True
...
def pickup_next_lane_work(
    ...
    *,
    ...
    rows = conn.execute(
        """
        SELECT * FROM lane_work_items WHERE status IN ('open', 'blocked')
        ORDER BY priority DESC, failed_required_ci DESC, stale_open_pr DESC,
                 governance_blocker DESC, review_gap DESC,
                 dependency_unblocked DESC, created_at ASC
        """
    ).fetchall()
    for row in rows:
        ...
        claim = claim_lane_work_item(
            conn,
            row["work_item_id"],
            lane_id=lane_id,
            claim_owner=agent_session_id,
            ttl_seconds=ttl_seconds,
            evidence_path=evidence,
            now=ts,
        )
        if claim.get("claimed"):
            heartbeat = record_lane_heartbeat(
                conn,
                lane_id=lane_id,
            ...
            item = dict(row)
            item.update({"claim": claim, "heartbeat": heartbeat})
            return item
    record_lane_heartbeat(
        conn,
        lane_id=lane_id,
        agent_session_id=agent_session_id,
        repo_scope=authorized_scopes[0] if authorized_scopes else "",
        state="idle-no-work",
        now=ts,
    )
    return None
...
def lane_status_report(
    ...
    for row in conn.execute("SELECT * FROM lane_heartbeats ORDER BY lane_id").fetchall():
        ...
        if row["state"] == "refused-closeout":
            ...
            "blocked", "awaiting-human", "idle-no-work", "working", "failed-heartbeat",
        }:
            classification = row["state"]
        stale_claims = conn.execute(
            "SELECT COUNT(*) AS n FROM lane_claims WHERE lane_id=? "
            "AND claim_status='active' AND claim_expires_at < ?",
            (row["lane_id"], ts),
        ).fetchone()["n"]
        lanes.append({
            "lane_id": row["lane_id"],
