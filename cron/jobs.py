...
def _output_rotation_disabled() -> bool:
    """Honor the global and cron-output-specific retention kill switches."""
    return any(
...
def _output_keep_last() -> int:
    """Resolve the per-job cap, with the environment override taking precedence."""
    if _output_rotation_disabled():
        return 0
    raw = os.environ.get("HERMES_CRON_OUTPUT_KEEP", "").strip()
    if not raw:
        return max(0, _cron_output_keep())
    try:
...
def _rotate_job_output(job_output_dir: Path, keep: int) -> None:
    """Delete oldest outputs beyond the cap, subject to the age floor."""
    if keep <= 0:
        return
    try:
...
def save_job_output(job_id: str, output: str):
    ...
    if "HERMES_CRON_OUTPUT_KEEP" in os.environ or _output_rotation_disabled():
