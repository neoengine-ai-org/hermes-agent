...
The adapter is advisory. It publishes the stable contexts `Hermes CI required`,
...
Pull requests from forks fail closed before candidate execution, and the workflow
does not use a private cross-repository checkout or token.
...
Duration artifacts and uv caching are enabled only for protected `main` pushes.

Review routing follows the canonical model: R0-R2 use no CI model; R3 requires
one post-green adversarial receipt or an explicit `DEGRADED` fallback; R4-R5
remain blocked for protected specialist review. The adapter never grants those
reviews itself.
