...
def test_close_claim_updates_terminal_work_item_status_and_prevents_reclaim(tmp_path: Path) -> None:
    store = DevLaneStore(tmp_path)
    store.register_lane("lane-a", repo_scope="neoengine-ai-org/neowealth", authorized_scopes=["repo/**"])
    store.add_work_item(
        {
            "work_item_id": "work-done",
            "repo_scope": "neoengine-ai-org/neowealth",
            "authorized_scopes": ["repo/app/page.tsx"],
            "status": "queued",
            "events": [{"event_type": "explicit_operator_command", "created_at": "2026-06-24T00:00:00Z"}],
    ...
    store.claim_work("work-done", "lane-a", "session-a", "2026-06-24T00:01:00Z", 60, "claims/work-done.json")

    store.close_claim("work-done", "completed", "receipts/work-done.json", "2026-06-24T00:02:00Z")
    pickup = store.pick_next_work("lane-a", "session-b", now="2026-06-24T00:03:00Z")

    assert store.read_json("work/items.json")["work-done"]["status"] == "completed"
    assert pickup["action"] == "idle-no-work"
